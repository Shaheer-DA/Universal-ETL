...
def run_etl_job(self, db_config, query_config):
    ...
    # NO TRY-EXCEPT HERE -> Let Celery handle errors natively
    ...
    self.update_state(state="PROGRESS", meta={"status": "Cleaning...", "progress": 80})
    ...
    df_leads_clean, df_leads_dupe = process_df(all_leads, ["PAN", "Mobile"])

    valid_ids = (
        set(df_leads_clean["Customer_ID"]) if not df_leads_clean.empty else set()
    )
    loans_clean_list = [x for x in all_loans if x["Customer_ID"] in valid_ids]
    enqs_clean_list = [x for x in all_enqs if x["Customer_ID"] in valid_ids]
    an_clean_list = [x for x in all_analysis if x["Customer_ID"] in valid_ids]
    ...
    loans_dupe_list = [x for x in all_loans if x["Customer_ID"] in dupe_ids]
    enqs_dupe_list = [x for x in all_enqs if x["Customer_ID"] in dupe_ids]
    an_dupe_list = [x for x in all_analysis if x["Customer_ID"] in dupe_ids]
    ...
    def strict_format(df, is_lead=False):
        if df.empty:
            return df
        cols = list(df.columns)
        found_start = [c for c in start_cols if c in cols]
        found_end = ["Source_DB"] if "Source_DB" in cols else []
        glance_cols = [
            "Has_Home_Loan",
            "Has_Auto_Loan",
            "Has_Credit_Card",
        ...
        found_glance = [c for c in glance_cols if c in cols]

        used = set(found_start + found_end + found_glance)
        middle = [c for c in cols if c not in used]
        return df[found_start + found_glance + middle + found_end]
    ...
    def write_file(path, d_l, d_a, d_lo, d_e):
        with pd.ExcelWriter(path, engine="openpyxl") as writer:
            d_l.pipe(strict_format).to_excel(writer, sheet_name="Leads", index=False)
    ...
    if not df_leads_dupe.empty:
        write_file(dupe_path, df_leads_dupe, df_an_dupe, df_loans_dupe, df_enqs_dupe)
        res["dupe_path"] = dupe_path
